...
class Calendrier(gridlib.Grid, glr.GridWithLabelRenderersMixin): 
    ...
    def OnLabelRightClick(self, event):
        ...
        # Item Copier
        item = wx.MenuItem(menuPop, 2000+numLigne, u"Copier")
        ...
        # Item Coller
        item = wx.MenuItem(menuPop, 3000+numLigne, u"Coller")
        ...
        # Item Reinitialiser
        item = wx.MenuItem(menuPop, 4000+numLigne, u"Réinitialiser cette date")
    ...
    def SaisieLot(self):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try :
                dlgAttente = PBI.PyBusyInfo(u"Veuillez patienter durant l'opération...", parent=None, title=u"Patientez...", icon=wx.Bitmap("Images/16x16/Logo.png", wx.BITMAP_TYPE_ANY))
                wx.Yield() 
                ...
                dlg2 = wx.MessageDialog(self, u"Désolé, le problème suivant a été rencontré dans le traitement par lot des ouvertures : \n\n%s" % err, u"Erreur", wx.OK | wx.ICON_ERROR)
                dlg2.ShowModal()
                dlg2.Destroy()
        dlg.Destroy()
    ...
    def ReInitDate(self, event):
        ...
        dlg = wx.MessageDialog(self, u"Souhaitez-vous vraiment réinitialiser les paramètres de la date du %s ?" % DateComplete(dateDD), u"Réinitialisation", wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
    ...
    def GetListeGroupes(self):
        ...
        if len(listeGroupes) == 0 :
            listeGroupes.append({ "IDgroupe" : 0, "nom" : u"Sans groupe" })
        # Tous les groupes
        if self.afficherTousGroupes == True :
            listeGroupes.append({ "IDgroupe" : None, "nom" : u"Total max." })
        return listeGroupes
...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDactivite=None):
        ...
        intro = u"Ce calendrier vous permet de définir les jours d'ouverture de l'activité et le nombre maximal de places. Cliquez sur les cases pour indiquer que l'unité est fonctionnelle et saisissez directement les effectifs maximals en double-cliquant sur les cases blanches. <U>Important :</U> Cliquez avec le bouton droit de la souris sur les cases Dates pour utiliser le Copier-Coller."
        titre = u"Calendrier des ouvertures"
        self.ctrl_bandeau = CTRL_Bandeau.Bandeau(self, titre=titre, texte=intro, hauteurHtml=30, nomImage="Images/32x32/Calendrier.png")
        
        # Selection Mois
        self.staticbox_mois_staticbox = wx.StaticBox(self, -1, u"Sélection du mois")
        ...
        # Légende
        self.staticbox_legende_staticbox = wx.StaticBox(self, -1, u"Légende")
        self.listeLegende = [
            { "label" : u"Ouvert", "couleur" : COULEUR_OUVERTURE, "ctrl_label" : None, "ctrl_img" : None },
            { "label" : u"Fermé", "couleur" : COULEUR_FERMETURE, "ctrl_label" : None, "ctrl_img" : None },
            { "label" : u"Vacances", "couleur" : COULEUR_VACANCES, "ctrl_label" : None, "ctrl_img" : None },
            { "label" : u"Férié", "couleur" : COULEUR_FERIE, "ctrl_label" : None, "ctrl_img" : None },
            { "label" : u"Effectifs max.", "couleur" : (255, 255, 255), "ctrl_label" : None, "ctrl_img" : None },
            ]
        index = 0
        ...
        self.check_tous_groupes = wx.CheckBox(self, -1, u"Afficher Total max.")
        ...
        self.bouton_saisie_lot = wx.BitmapButton(self, -1, wx.Bitmap("Images/BoutonsImages/Saisie_lot.png", wx.BITMAP_TYPE_ANY))
    ...
    def __set_properties(self):
        self.SetTitle(u"Calendrier des ouvertures")
        self.ctrl_mois.SetToolTipString(u"Sélectionnez un mois")
        self.ctrl_annee.SetMinSize((70, -1))
        self.ctrl_annee.SetToolTipString(u"Sélectionnez une année")
        self.check_tous_groupes.SetToolTipString(u"Cochez cette case pour afficher les lignes 'Total max.' afin de saisir des limitations d'effectifs globales")
        self.bouton_aide.SetToolTipString(u"Cliquez ici pour obtenir de l'aide")
        self.bouton_saisie_lot.SetToolTipString(u"Cliquez ici saisir ou supprimer un lot d'ouvertures")
    ...
    def OnBoutonOk(self, event):
        try :
            dlgAttente = PBI.PyBusyInfo(u"Veuillez patienter durant la sauvegarde des données...", parent=None, title=u"Enregistrement", icon=wx.Bitmap("Images/16x16/Logo.png", wx.BITMAP_TYPE_ANY))
            wx.Yield() 
            ...
            del dlgAttente
            traceback.print_exc(file=sys.stdout)
            dlg = wx.MessageDialog(self, u"Désolé, le problème suivant a été rencontré dans la sauvegarde des ouvertures : \n\n%s" % err, u"Erreur", wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
