...
class ListView(FastObjectListView):
        ...
        for nomFichier in listeFichiers :
            if nomFichier.startswith("actions_%s" % IDfichier) and (nomFichier.endswith(".dat") or (nomFichier.endswith(".archive") and self.inclure_archives == True)) :
                ...
                if len(listeConsommations) > 0 :
                    liste_details_actions.append(u"%d actions sur les consommations" % len(listeConsommations))
                ...
                if len(listeMemosJournees) > 0 :
                    liste_details_actions.append(u"%d actions sur les mémos journaliers" % len(listeMemosJournees))

                DB.Close() 
                
                detail_actions = ", ".join(liste_details_actions)
                if detail_actions == "" :
                    detail_actions = u"Aucune action"
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(u"Date génération", "left", 130, "horodatage", typeDonnee="texte", stringConverter=FormateHorodatage),
            ColumnDefn(u"Appareil (ID)", "left", 130, "appareil", typeDonnee="texte"),
            ColumnDefn(u"Contenu du fichier", "left", 285, "detail_actions", typeDonnee="texte"),
            ColumnDefn(u"Nom du fichier", "left", 285, "nom_fichier", typeDonnee="texte"),
            ColumnDefn(u"Taille", "left", 70, "taille_fichier", typeDonnee="entier", stringConverter=FormateTailleFichier),
            ]

        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.CreateCheckStateColumn(0)
        self.SetEmptyListMsg(u"Aucun fichier")
    ...
    def OnContextMenu(self, event):
        ...
        # Item Archiver
        item = wx.MenuItem(menuPop, 10, u"Archiver le fichier")
    ...
    def Archiver(self, event):
        dlg = wx.MessageDialog(self, u"Souhaitez-vous vraiment archiver ce fichier ?", u"Archivage", wx.YES_NO|wx.CANCEL|wx.NO_DEFAULT|wx.ICON_QUESTION)
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=u"Liste des fichiers à synchroniser", format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def ExportTexte(self, event):
        import UTILS_Export
        UTILS_Export.ExportTexte(self, titre=u"Liste des fichiers à synchroniser")
        
    def ExportExcel(self, event):
        import UTILS_Export
        UTILS_Export.ExportExcel(self, titre=u"Liste des fichiers à synchroniser")
