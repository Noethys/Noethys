...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        self.SetEmptyListMsg(u"Aucun type de cotisation")
    ...
    def OnContextMenu(self, event):
        ...
        # Item Par défaut
        item = wx.MenuItem(menuPop, 60, u"Définir comme type de cotisation par défaut")
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=u"Liste des types de cotisations", format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, u"Vous n'avez sélectionné aucun type de cotisation à modifier dans la liste !", u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, u"Vous n'avez sélectionné aucun type de cotisation à supprimer dans la liste !", u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDtype_cotisation = self.Selection()[0].IDtype_cotisation
        ...
        if nbreCotisations > 0 :
            dlg = wx.MessageDialog(self, u"Ce type de cotisation a déjà été attribué à %d cotisations(s).\n\nVous ne pouvez donc pas le supprimer !" % nbreCotisations, u"Suppression impossible", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        DB.Close()
        if nbreActivites > 0 :
            dlg = wx.MessageDialog(self, u"Ce type de cotisation a déjà été attribué à %d activité(s) en tant que pièce à fournir.\n\nVous ne pouvez donc pas le supprimer !" % nbreActivites, u"Suppression impossible", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        dlg = wx.MessageDialog(self, u"Souhaitez-vous vraiment supprimer ce type de cotisation ?", u"Suppression", wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
    ...
    def SetDefaut(self, event):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("parametrage_types_cotisations", "modifier") == False : return
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, u"Vous n'avez sélectionné aucun type de cotisation dans la liste !", u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(u"Rechercher un type de cotisation...")
