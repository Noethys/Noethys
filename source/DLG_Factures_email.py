...
class Dialog(wx.Dialog):
    def __init__(self, parent, filtres=[]):
        ...
        # Bandeau
        intro = u"Cochez les factures à envoyer par Email puis cliquez sur le bouton 'Transférer vers l'éditeur d'Emails'."
    ...
    def __set_properties(self):
        ...
        self.bouton_ok.SetToolTipString(u"Cliquez ici pour transférer les factures vers l'éditeur d'Emails")
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(tracks) == 0 : 
            dlg = wx.MessageDialog(self, u"Vous n'avez sélectionné aucune facture à envoyer par Email !", u"Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if len(listeEnvoiNonDemande) > 0 :
            ...
            texte += u"Souhaitez-vous quand même leur envoyer une facture ?"
            dlg = wx.MessageDialog(self, texte, u"Avertissement", wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal() 
            dlg.Destroy()
            if reponse != wx.ID_YES :
        ...
        if len(listeDonnees) == 0 : 
            dlg = wx.MessageDialog(self, u"Il ne reste finalement aucune facture à envoyer par Email !", u"Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            SupprimerFichiersTemp()
            return
