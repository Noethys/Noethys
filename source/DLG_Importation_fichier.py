...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        ...
        # Liste fichiers
        self.box_fichiers_staticbox = wx.StaticBox(self, -1, u"1. Sélection du fichier")
        self.ctrl_fichiers = CTRL_Liste_fichiers.CTRL(self, mode="local")
        self.ctrl_fichiers.SetMinSize((-1, 200))

        # Options
        self.box_options_staticbox = wx.StaticBox(self, -1, u"2. Sélection des données optionnelles")
    ...
    def OnBoutonOk(self, event): 
        ...
        if nbreFamilles > 0 :
            dlg = wx.MessageDialog(self, u"Attention, %d fiches familles sont déjà saisies dans ce fichier. Si vous utilisez la fonction d'importation, TOUTES les fiches actuelles seront écrasées !!!\n\nSouhaitez-vous tout de même continuer ?" % nbreFamilles, u"Avertissement", wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
            if dlg.ShowModal() != wx.ID_YES :
                dlg.Destroy()
                return False
            dlg.Destroy()
        
        dlg = wx.MessageDialog(self, u"Souhaitez-vous vraiment lancer l'importation des familles du fichier '%s' ?\n\nAttention, toutes les données actuelles seront écrasées !" % titre, u"Confirmation", wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_EXCLAMATION)
        if dlg.ShowModal() != wx.ID_YES :
            dlg.Destroy()
            return False
        dlg.Destroy()
        ...
        # Fin
        nbreFamilles = self.GetNbreFamilles() 
        dlg = wx.MessageDialog(self, u"La procédure d'importation est terminée !\n\n%d familles ont été importées avec succès." % nbreFamilles, u"Confirmation", wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
