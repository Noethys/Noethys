...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ColumnDefn(u"ID", "left", 0, "IDlot", typeDonnee="entier"),
            ColumnDefn(u"Nom", "left", 290, "nom", typeDonnee="texte"), 
            ColumnDefn(u"Nbre de factures", "left", 110, "nbreFactures", typeDonnee="entier"), 
            ]
        
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(u"Aucun lot de factures")
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=u"Liste des lots de factures", format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Ajouter(self, event):
        ...
        dlg = wx.TextEntryDialog(self, u"Saisissez le nom du nouveau lot de factures :", u"Saisie d'un nouveau lot", u"")
    ...
    def Modifier(self, event):
        ...
        dlg = wx.TextEntryDialog(self, u"Modifiez le nom du lot de factures :", u"Modification d'un lot", nom)
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
                dlg = wx.MessageDialog(self, u"Le nom que vous avez saisi n'est pas valide.", u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, u"Vous n'avez sélectionné aucun lot de factures à supprimer dans la liste !", u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        nbreFactures = self.Selection()[0].nbreFactures
        if nbreFactures > 0 :
            dlg = wx.MessageDialog(self, u"Il est impossible de supprimer ce lot de factures puisqu'il est déjà constitué de %d facture(s) !" % nbreFactures, u"Suppression impossible", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, u"Souhaitez-vous vraiment supprimer ce lot de factures ?", u"Suppression", wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(u"Rechercher un lot de factures...")
