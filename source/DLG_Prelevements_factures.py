...
class Dialog(wx.Dialog):
    def __init__(self, parent, IDlot=None):
        ...
        intro = u"Cochez les factures que vous souhaitez ajouter au lot de prélèvements puis cliquez sur le bouton 'Ok'."
        titre = u"Ajouter des factures au prélèvement"
        self.SetTitle(titre)
    ...
    def OnBoutonOk(self, event):
        ...
        if len(tracks) == 0 :
            dlg = wx.MessageDialog(self, u"Vous n'avez sélectionné aucune facture à inclure dans votre lot de prélèvements !", u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
            ...
            if dictFacturesPrelevees.has_key(track.IDfacture) :
                for dictTemp in dictFacturesPrelevees[track.IDfacture] :
                    if dictTemp["IDlot"] != self.IDlot :
                        ...
                        listeAutresLots.append(u"- %s (%s) avec le statut '%s'" % (nomPrelevement, datePrelevement, statut.capitalize()))
            
            if len(listeAutresLots) > 0 :
                message1 = u"La facture n°%s est déjà présente dans les autres lots de prélèvements suivants. Souhaitez-vous tout de même l'inclure de votre lot de prélèvements actuel ?" % track.numero
        ...
        if len(listeErreursSoldes) > 0 :
            dlg = wx.MessageDialog(self, u"Vous avez sélectionné %d factures qui ont déjà été payées !" % len(listeErreursSoldes), u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
