...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(u"Date début", "left", 120, "date_debut", typeDonnee="date", stringConverter=FormateDate),
            ColumnDefn(u"Date fin", "left", 120, "date_fin", typeDonnee="date", stringConverter=FormateDate),
            ]

        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(u"Aucun exercice")
    ...
    def OnContextMenu(self, event):
        ...
        # Item Par défaut
        item = wx.MenuItem(menuPop, 60, u"Définir comme exercice par défaut")
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=u"Liste des exercices", format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, u"Vous n'avez sélectionné aucun exercice à modifier dans la liste !", u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, u"Vous n'avez sélectionné aucun exercice à supprimer dans la liste !", u"Erreur de saisie", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if nbreVentilations > 0 :
            dlg = wx.MessageDialog(self, u"Cet exercice a déjà été attribué à %d ventilation(s) d'opération(s).\n\nVous ne pouvez donc pas le supprimer !" % nbreVentilations, u"Suppression impossible", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Suppression
        dlg = wx.MessageDialog(self, u"Souhaitez-vous vraiment supprimer cet exercice ?", u"Suppression", wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
