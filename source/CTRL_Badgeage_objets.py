...
class Barre_numerique(Objet):
    def __init__(self, parent, *args, **kwds):
        Objet.__init__(self, parent, *args, **kwds)
    def Draw(self):
        self.bmp = wx.Bitmap("Images/Badgeage/%s_barrenum.png" % self.interface.GetStyle())
...
class Fond_clavier_numerique(Objet):
    ...
    def Draw(self):
        ...
        self.bmp = wx.Bitmap("Images/Badgeage/%s_claviernum.png" % self.interface.GetStyle())
        self.taille = self.bmp.GetSize()
        self.dc.DrawBitmap(self.bmp, self.position[0], self.position[1])
...
class Groupe_clavier_numerique(Groupe):
    ...
    def __init__(self, parent, *args, **kwds):
        ...
        for nomTouche in self.listeTouches :
            ...
            bmp = wx.Bitmap("Images/Badgeage/%s.png" % nomTouche, wx.BITMAP_TYPE_PNG)
            image = bmp.ConvertToImage()
...
class Fond_liste_individus(Objet):
    ...
    def Draw(self):
        # Dessin du fond du clavier numérique
        self.bmp = wx.Bitmap("Images/Badgeage/%s_listeindividus.png" % self.interface.GetStyle())
        self.taille = self.bmp.GetSize() 
        self.dc.DrawBitmap(self.bmp, self.position[0], self.position[1])
