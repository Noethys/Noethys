...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(u"Nom", 'left', 200, "label", typeDonnee="texte", isSpaceFilling=True),
            ColumnDefn(u"Numéro", 'left', 0, "numero", typeDonnee="texte"),
            ]
        
        self.rowFormatter = rowFormatter
        self.SetColumns(liste_Colonnes)
        self.SetEmptyListMsg(u"Aucun compte")
    ...
    def Apercu(self, event):
        ...
        prt = UTILS_Printer.ObjectListViewPrinter(self, titre=u"Liste des comptes comptables", format="A", orientation=wx.PORTRAIT)
        prt.Preview()
    ...
    def Supprimer(self, event):
        ...
        DB.Close()
        if nbreCategorie > 0 :
            dlg = wx.MessageDialog(self, u"Ce compte a déjà été attribué à %d catégorie(s).\n\nVous ne pouvez donc pas le supprimer !" % nbreCategorie, u"Suppression impossible", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return

        # Suppression
        dlg = wx.MessageDialog(self, u"Souhaitez-vous vraiment supprimer ce compte ?", u"Suppression", wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
