...
class Dialog(wx.Dialog):
    def __init__(self, parent, filtres=[]):
        ...
        # Bandeau
        intro = _(u"Cochez les locations à envoyer par Email puis cliquez sur le bouton 'Transférer vers l'éditeur d'Emails'.")
        titre = _(u"Envoi de locations par Email")
        self.SetTitle(titre)
    ...
    def OnBoutonOk(self, event): 
        ...
        if len(listeDonnees) == 0 : 
            dlg = wx.MessageDialog(self, _(u"Il ne reste finalement aucune location à envoyer par Email !"), _(u"Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            SupprimerFichiersTemp()
            return
