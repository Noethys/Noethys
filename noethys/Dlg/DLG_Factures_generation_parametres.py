...
class CTRL_Prefixe_factures(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        self.dictDonnees = {}
        self.dictDonnees[0] = {"ID": None, "nom": _(
            u"Aucun préfixe"), "prefixe": None}
        index = 1
        for IDprefixe, nom, prefixe in listeDonnees:
            self.dictDonnees[index] = {
                "ID": IDprefixe, "nom": nom, "prefixe": prefixe}
...
class CTRL_Famille(wx.Choice):
    ...
    def GetListeDonnees(self):
        ...
        for IDfamille, dictTemp in titulaires.items():
            listeFamilles.append(
                (dictTemp["titulairesSansCivilite"], IDfamille, dictTemp["IDcompte_payeur"]))
        listeFamilles.sort()
        listeItems = [u"",]
        self.dictDonnees = {}
        self.dictDonnees[0] = {"IDfamille": 0, "nom": _(
            u"Inconnue"), "IDcompte_payeur": 0}
        index = 1
        for nom, IDfamille, IDcompte_payeur in listeFamilles:
            self.dictDonnees[index] = {
                "IDfamille": IDfamille, "nom ": nom, "IDcompte_payeur": IDcompte_payeur}
            listeItems.append(nom)
            index += 1
        return listeItems
...
class Panel(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(
            self, parent, id=-1, name="DLG_Factures_generation_parametres", style=wx.TAB_TRAVERSAL)
        self.parent = parent
        ...
        self.bouton_lots = wx.BitmapButton(
            self, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Mecanisme.png"), wx.BITMAP_TYPE_ANY))
        ...
        self.bouton_prefixes = wx.BitmapButton(
            self, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Mecanisme.png"), wx.BITMAP_TYPE_ANY))
        self.label_prochain_numero = wx.StaticText(
            self, -1, _(u"Prochain numéro :"))
        self.ctrl_prochain_numero = wx.TextCtrl(self, -1, u"", size=(95, -1))
        self.bouton_prochain_numero = wx.BitmapButton(
            self, -1, wx.Bitmap(Chemins.GetStaticPath(u"Images/16x16/Actualiser2.png"), wx.BITMAP_TYPE_ANY))
        self.check_numero_auto = wx.CheckBox(self, -1, _(u"Auto."))
        self.box_parametres_staticbox = wx.StaticBox(
            self, -1, _(u"Paramètres"))
        self.label_date_emission = wx.StaticText(
            self, -1, _(u"Date d'émission :"))
        self.ctrl_date_emission = CTRL_Saisie_date.Date2(self)
        self.label_date_echeance = wx.StaticText(
            self, -1, _(u"Date d'échéance :"))
        ...
        self.radio_familles_toutes = wx.RadioButton(
            self, -1, _(u"Toutes les familles"), style=wx.RB_GROUP)
    ...
    def __set_properties(self):
        self.ctrl_prefixe.SetToolTip(wx.ToolTip(
            _(u"Sélectionnez un préfixe de numéro de factures dans la liste proposée. Cet paramètre permet d'obtenir des numéros de facture de type 'ABC-00001'.")))
        self.bouton_prefixes.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour accéder à la gestion des préfixes de factures")))
        self.ctrl_lot.SetToolTip(wx.ToolTip(
            _(u"Sélectionnez un nom de lot à associer aux factures générées. Ex : Janvier 2013, Février, 2013, etc... Ce nom vous permettra de retrouver vos factures facilement [Optionnel]")))
        self.bouton_lots.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour accéder à la gestion des lots")))
        self.ctrl_prochain_numero.SetToolTip(wx.ToolTip(
            _(u"Numéro de la prochaine facture générée. Vous pouvez modifier ce numéro si vous souhaitez par exemple modifier la numérotation en début d'année")))
        self.bouton_prochain_numero.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour sélectionner automatiquement le prochain numéro de facture")))
        self.check_numero_auto.SetToolTip(wx.ToolTip(
            _(u"Cochez cette case pour laisser Noethys sélectionner automatiquement le prochain numéro de facture")))
        self.ctrl_date_emission.SetToolTip(wx.ToolTip(
            _(u"Saisissez ici la date d'émission la date des factures (Par défaut la date du jour)")))
        self.ctrl_date_echeance.SetToolTip(wx.ToolTip(
            _(u"Saisissez la date d'échéance de paiement qui apparaîtra sur la facture [Optionnel]")))
        self.ctrl_mention1.SetToolTip(
            wx.ToolTip(_(u"Saisissez une mention libre")))
        self.ctrl_mention2.SetToolTip(
            wx.ToolTip(_(u"Saisissez une mention libre")))
        self.ctrl_mention3.SetToolTip(
            wx.ToolTip(_(u"Saisissez une mention libre")))
        self.check_consommations.SetToolTip(wx.ToolTip(
            _(u"Cochez cette case pour inclure les prestations de consommations")))
        self.check_cotisations.SetToolTip(wx.ToolTip(
            _(u"Cochez cette case pour inclure les prestations de cotisations")))
        self.check_locations.SetToolTip(wx.ToolTip(
            _(u"Cochez cette case pour inclure les prestations de locations")))
        self.check_autres.SetToolTip(wx.ToolTip(
            _(u"Cochez cette case pour inclure les autres types de prestations")))
        self.check_inclure_cotisations_si_conso.SetToolTip(wx.ToolTip(
            _(u"Les cotisation ne seront intégrées à la facture que si la famille a des consommations sur les activités sélectionnées")))
        self.radio_familles_toutes.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour rechercher les factures de toutes les familles (par défaut)")))
        self.radio_familles_unique.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour rechercher les factures d'une seule famille")))
        self.ctrl_famille.SetToolTip(wx.ToolTip(
            _(u"Sélectionnez ici une famille")))
        self.check_prestations_anterieures.SetToolTip(wx.ToolTip(
            _(u"Cochez cette case pour inclure les prestations antérieures à la date de début non facturées depuis la date souhaitée")))
    ...
    def __do_layout(self):
        ...
        # Période
        box_periode = wx.StaticBoxSizer(
            self.box_periode_staticbox, wx.VERTICAL)
        grid_sizer_periode = wx.FlexGridSizer(rows=1, cols=4, vgap=5, hgap=5)
        grid_sizer_periode.Add(self.label_date_debut, 0,
                               wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_periode.Add(self.ctrl_date_debut, 0, 0, 0)
        grid_sizer_periode.Add(self.label_date_fin, 0,
                               wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)
        ...
        grid_sizer_lot.Add(self.label_lot, 0, wx.ALIGN_RIGHT |
                           wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_lot.Add(self.ctrl_lot, 0, wx.EXPAND |
                           wx.ALIGN_CENTER_VERTICAL, 0)
        ...
        box_parametres = wx.StaticBoxSizer(
            self.box_parametres_staticbox, wx.VERTICAL)
        grid_sizer_parametres = wx.FlexGridSizer(
            rows=7, cols=2, vgap=5, hgap=5)

        grid_sizer_parametres.Add(
            self.label_prefixe, 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)

        grid_sizer_prefixe = wx.FlexGridSizer(rows=1, cols=2, vgap=5, hgap=5)
        grid_sizer_prefixe.Add(self.ctrl_prefixe, 0,
                               wx.EXPAND | wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_prefixe.Add(self.bouton_prefixes, 0,
                               wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_prefixe.AddGrowableCol(0)
        grid_sizer_parametres.Add(grid_sizer_prefixe, 1, wx.EXPAND, 0)

        grid_sizer_parametres.Add(
            self.label_prochain_numero, 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)

        grid_sizer_numero = wx.FlexGridSizer(rows=1, cols=3, vgap=5, hgap=5)
        grid_sizer_numero.Add(self.ctrl_prochain_numero,
                              0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 0)
        grid_sizer_numero.Add(self.bouton_prochain_numero,
                              0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 0)
        grid_sizer_numero.Add(self.check_numero_auto, 0,
                              wx.ALIGN_CENTER_VERTICAL | wx.ALL, 0)
        grid_sizer_parametres.Add(grid_sizer_numero, 0, 0, 0)

        grid_sizer_parametres.Add(
            self.label_date_emission, 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_parametres.Add(self.ctrl_date_emission, 0, 0, 0)
        grid_sizer_parametres.Add(
            self.label_date_echeance, 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_parametres.Add(self.ctrl_date_echeance, 0, 0, 0)
        grid_sizer_parametres.Add(
            self.label_mention1, 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_parametres.Add(self.ctrl_mention1, 0, wx.EXPAND, 0)
        grid_sizer_parametres.Add(
            self.label_mention2, 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_parametres.Add(self.ctrl_mention2, 0, wx.EXPAND, 0)
        grid_sizer_parametres.Add(
            self.label_mention3, 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL, 0)
        ...
        # Familles
        box_familles = wx.StaticBoxSizer(
            self.box_familles_staticbox, wx.VERTICAL)
        ...
        # Activités
        box_activites = wx.StaticBoxSizer(
            self.box_activites_staticbox, wx.VERTICAL)

        box_activites.Add(self.ctrl_activites, 1, wx.ALL | wx.EXPAND, 10)
        grid_sizer_droit.Add(box_activites, 1, wx.EXPAND, 0)

        # Elements
        box_elements = wx.StaticBoxSizer(
            self.box_elements_staticbox, wx.VERTICAL)
        ...
        box_elements.Add(grid_sizer_elements, 1, wx.LEFT |
                         wx.RIGHT | wx.TOP | wx.EXPAND, 10)
        box_elements.Add((5, 5), 0, wx.EXPAND, 0)
        box_elements.Add(self.check_inclure_cotisations_si_conso,
                         1, wx.LEFT | wx.RIGHT | wx.BOTTOM | wx.EXPAND, 10)
        grid_sizer_droit.Add(box_elements, 1, wx.EXPAND, 0)

        # Inclure prestations antérieures
        box_anterieures = wx.StaticBoxSizer(
            self.box_anterieures_staticbox, wx.VERTICAL)
        grid_sizer_anterieures = wx.FlexGridSizer(
            rows=1, cols=3, vgap=2, hgap=2)
        grid_sizer_anterieures.Add(
            self.check_prestations_anterieures, 0, wx.EXPAND, 0)
    ...
    def OnCheckAnterieures(self, event=None):
        self.ctrl_date_anterieures.Enable(
            self.check_prestations_anterieures.GetValue())
    ...
    def Validation(self):
        ...
        if self.ctrl_date_debut.FonctionValiderDate() == False or date_debut == None:
            dlg = wx.MessageDialog(self, _(u"La date de début de période ne semble pas valide !"), _(
        ...
        if self.ctrl_date_fin.FonctionValiderDate() == False or date_fin == None:
            dlg = wx.MessageDialog(self, _(u"La date de fin de période ne semble pas valide !"), _(
                u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if date_debut > date_fin:
            dlg = wx.MessageDialog(self, _(u"La date de début de période est supérieure à la date de fin !"), _(
                u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            self.ctrl_date_fin.SetFocus()
        ...
        if gestion.IsPeriodeinPeriodes("factures", date_debut, date_fin) == False:
            return False
        ...
        if IDlot == None:
            dlg = wx.MessageDialog(self, _(u"Vous n'avez pas sélectionné de lot de factures à associer.\n\nSouhaitez-vous quand même continuer ?"), _(
                u"Confirmation"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
        ...
        if self.check_numero_auto.GetValue() == True:
            ...
            if prochain_numero < self.prochain_numero_defaut:
                dlg = wx.MessageDialog(self, _(u"Le prochain numéro de facture n'est pas valide : une facture générée porte déjà un numéro égal ou supérieur !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_prochain_numero.SetFocus()
        ...
        if self.ctrl_date_emission.FonctionValiderDate() == False or date_emission == None:
            dlg = wx.MessageDialog(self, _(u"La date d'émission ne semble pas valide !"), _(
                u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.check_autres.GetValue() == True:
            prestations.append("autre")
        if len(prestations) == 0:
            dlg = wx.MessageDialog(self, _(u"Vous devez obligatoirement cocher au moins un type de prestation à facturer !"), _(
                u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        ...
        if self.radio_familles_toutes.GetValue() == True:
            ...
            if IDcompte_payeur_unique == None:
                dlg = wx.MessageDialog(self, _(u"Vous avez sélectionné l'option 'famille unique' mais sans sélectionner de famille dans la liste proposée !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
        ...
        if len(listeActivites) == 0:
            dlg = wx.MessageDialog(self, _(u"Vous n'avez sélectionné aucune activité.\n\nSouhaitez-vous quand même continuer ?"), _(
                u"Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
            resultat = dlg.ShowModal()
            dlg.Destroy()
            if resultat != wx.ID_YES:
                return False
        ...
        if self.check_prestations_anterieures.GetValue() == True:
            ...
            if self.ctrl_date_anterieures.FonctionValiderDate() == False or date_anterieure == None:
                dlg = wx.MessageDialog(self, _(u"La date antérieure ne semble pas valide !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_date_anterieures.SetFocus()
        ...
        for IDactivite in listeActivites:
            if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("facturation_factures", "creer", IDactivite=IDactivite, afficheMessage=False) == False:
                dlg = wx.MessageDialog(self, _(u"Vous n'avez pas l'autorisation de générer des factures pour l'ensemble des activités sélectionnées !"), _(
                    u"Action non autorisée"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
