...
class Panel(wx.Panel):
    def __init__(self, parent, IDactivite=None, nouvelleActivite=False):
        ...
        self.staticbox_regie_facturation_staticbox = wx.StaticBox(
            self, -1, _(u"Régie de facturation"))
        ...
        self.radio_logo_org = wx.RadioButton(
            self, -1, u"Identique à l'organisateur", style=wx.RB_GROUP)
        ...
        listePublic = [(1, _(u"Représentants"), False),
                       (2, _(u"Enfants"), False),]
    ...
    def __set_properties(self):
        self.ctrl_nom_complet.SetToolTip(wx.ToolTip(
            _(u"Saisissez ici le nom complet de l'activité")))
        self.ctrl_nom_abrege.SetMinSize((60, -1))
        self.ctrl_nom_abrege.SetToolTip(wx.ToolTip(
            _(u"Saisissez ici le nom abrégé de l'activité")))
        self.radio_coords_org.SetToolTip(wx.ToolTip(
            _(u"Selectionnez 'Autres coordonnées' si vous souhaitez attribuer des coordonnées différentes de celles de l'organisateur")))
        self.radio_coords_autres.SetToolTip(wx.ToolTip(
            _(u"Selectionnez 'Autres coordonnées' si vous souhaitez attribuer des coordonnées différentes de celles de l'organisateur")))
        self.ctrl_rue.SetToolTip(wx.ToolTip(
            _(u"Saisissez l'adresse de l'activité")))
        self.ctrl_site.SetToolTip(wx.ToolTip(
            _(u"Saisissez l'adresse du site internet de l'activité")))
        self.ctrl_responsables.SetToolTip(wx.ToolTip(
            _(u"Liste des responsables de l'activité")))
        ...
        self.bouton_modifier_responsable.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour modifier le nom sélectionné dans la liste")))
        self.bouton_supprimer_responsable.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour supprimer le nom sélectionné dans la liste")))
        self.bouton_defaut_responsable.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour définir le responsable sélectionné dans la liste par défaut")))
        self.radio_logo_org.SetToolTip(wx.ToolTip(
            _(u"Selectionnez 'Autre logo' pour attribuer à l'activité un logo différent de celui de l'organisateur")))
        self.radio_logo_autre.SetToolTip(wx.ToolTip(
            _(u"Selectionnez 'Autre logo' pour attribuer à l'activité un logo différent de celui de l'organisateur")))
        self.bouton_modifier_logo.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour ajouter ou modifier un logo pour cette activité")))
        ...
        self.bouton_visualiser_logo.SetToolTip(wx.ToolTip(
            _(u"Cliquez ici pour visualiser le logo en taille réelle")))
        self.radio_illimitee.SetToolTip(wx.ToolTip(
            _(u"Selectionnez 'Illimitee' si l'activité est un accueil de loisirs")))
        self.radio_limitee.SetToolTip(wx.ToolTip(
            _(u"Saisissez une periode de validité si l'activite fonctionne uniquement sur une période donnée")))
        self.ctrl_validite_du.SetToolTip(wx.ToolTip(
            _(u"Saisissez une date de debut de validité")))
        self.ctrl_validite_au.SetToolTip(wx.ToolTip(
            _(u"Saisissez ici une date de fin de validité")))
        self.ctrl_public.SetToolTip(wx.ToolTip(
            _(u"Cochez les publics autorisés à être inscrit à cette activité")))
        self.check_limitation_inscrits.SetToolTip(wx.ToolTip(
            _(u"Cochez cette case pour définir un nombre maximal d'inscrits pour cette activité (Utile uniquement pour les activités à durée limitée)")))
        self.ctrl_limitation_inscrits.SetToolTip(wx.ToolTip(
            _(u"Saisissez le nombre maximal d'inscrits de cette activité (Utile uniquement pour les activités à durée limitée)")))
        self.ctrl_code_comptable.SetToolTip(wx.ToolTip(
            _(u"Saisissez un code comptable si vous souhaitez utiliser l'export des écritures comptables vers des logiciels de compta")))
        self.ctrl_code_produit_local.SetToolTip(wx.ToolTip(
            _(u"Saisissez un code produit local si vous souhaitez utiliser l'export vers les logiciels de comptabilité publique")))
        self.ctrl_code_service.SetToolTip(wx.ToolTip(
            _(u"Saisissez un code service si vous souhaitez utiliser l'export vers les logiciels de comptabilité publique")))
        self.ctrl_code_analytique.SetToolTip(wx.ToolTip(
            _(u"Saisissez un code analytique si vous souhaitez utiliser l'export vers les logiciels de comptabilité")))
        self.ctrl_regie_facturation.SetToolTip(
            wx.ToolTip(_(u"Sélectionnez une régie de facturation")))
        self.check_inscriptions_multiples.SetToolTip(wx.ToolTip(
            _(u"Autoriser un individu à être inscrit plusieurs fois à la même activité. ATTENTION, ne pas utiliser cette option si vous utilisez des consommations dans cette activité car la grille des consommations est incompatible.")))
    ...
    def Importation(self):
        ...
        if resultat == 0:
            dlg = wx.MessageDialog(self, _(u"Il semble y avoir une erreur dans la dernière mise à niveau de la base de données. Essayez ceci depuis le Noethys de l'ordinateur qui héberge les données : Menu Outils > Utilitaires admin > Procédures. Saisissez le code A9281 et validez."), _(
    ...
    def Validation(self):
        ...
        if self.ctrl_nom_complet.GetValue() == "":
            dlg = wx.MessageDialog(self, _(u"Le nom de l'activité doit être obligatoirement saisi !"), _(
        ...
        if self.ctrl_nom_abrege.GetValue() == "":
            dlg = wx.MessageDialog(self, _(u"Le nom abrégé de l'activité doit être obligatoirement saisi !"), _(
                u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
        ...
        if self.radio_limitee.GetValue() == True:
            if self.ctrl_validite_du.Validation() == False:
                dlg = wx.MessageDialog(self, _(u"La date de début de validité ne semble pas valide !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if self.ctrl_validite_du.GetDate() == None:
                dlg = wx.MessageDialog(self, _(u"La date de début de validité n'a pas été spécifiée !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_validite_du.SetFocus()
            ...
            if self.ctrl_validite_au.Validation() == False:
                dlg = wx.MessageDialog(self, _(u"La date de fin de validité ne semble pas valide !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if self.ctrl_validite_au.GetDate() == None:
                dlg = wx.MessageDialog(self, _(u"La date de fin de validité n'a pas été spécifiée !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_validite_au.SetFocus()
            ...
            if self.ctrl_validite_du.GetDate() > self.ctrl_validite_au.GetDate():
                dlg = wx.MessageDialog(self, _(u"La date de validité de début ne peut pas être supérieure à la date de fin !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                self.ctrl_validite_du.SetFocus()
        ...
        if logo_org == 0:
            if self.ctrl_logo.GetBuffer() == None:
                dlg = wx.MessageDialog(self, _(u"Vous avez sélectionné un logo personnalisé pour cette activité mais vous n'en avez sélectionné aucun !"), _(
                    u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return False
