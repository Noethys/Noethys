...
class Dialog(wx.Dialog):
    ...
    def OnBoutonOk(self, event): 
        ...
        def SupprimerFichiersTemp():
            for IDdemande, fichier in dictPieces.iteritems() :
                os.remove(fichier)  
        ...
        for track in tracks :
            ...
            if adresse not in (None, "", []) : 
                if dictPieces.has_key(track.IDdemande) :
        ...
        if len(listeDonnees) == 0 : 
            dlg = wx.MessageDialog(self, _(u"Il ne reste finalement aucune demande à envoyer par Email !"), _(u"Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            SupprimerFichiersTemp()
            return
