...
def Procedure(code=""):
    ...
    if (code in DICT_PROCEDURES) == False:
        dlg = wx.MessageDialog(None, _(u"Désolé, cette procédure n'existe pas..."), _(
            u"Erreur"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return
    ...
    dlg = wx.MessageDialog(None, _(u"Souhaitez-vous vraiment lancer la procédure suivante ?\n\n   -> %s   ") %
                           titre, _(u"Lancement de la procédure"), wx.YES_NO | wx.YES_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
    ...
    try:
        ...
        dlg = wx.MessageDialog(None, _(
            u"Désolé, une erreur a été rencontrée :\n\n-> %s  ") % err, _(u"Erreur"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return
    # Fin
...
def A3687():
    ...
    for IDligne, qf_min, qf_max in listeLignes:
        if type(qf_min) == six.text_type or type(qf_min) == str:
            DB.ReqMAJ("tarifs_lignes", [
                      ("qf_min", float(qf_min.replace(",", "."))),], "IDligne", IDligne)
        if type(qf_max) == six.text_type or type(qf_max) == str:
            DB.ReqMAJ("tarifs_lignes", [
                      ("qf_max", float(qf_max.replace(",", "."))),], "IDligne", IDligne)
    DB.Close()
...
def X1234():
    ...
    dlg = wx.MultiChoiceDialog(None, _(u"Cochez les tables à exporter :"), _(
        u"Exportation vers la table DEFAUT"), listeTables)
...
def S1290():
    ...
    for IDtarif, IDcategorie_tarif, IDnom_tarif in listeTarifs:
        if IDcategorie_tarif != None:
            ...
            DB.ReqMAJ("tarifs", [("categories_tarifs", str(
                IDcategorie_tarif)),], "IDtarif", IDtarif)

            # Vide le champ IDcategorie_tarif de la table Tarifs
            DB.ReqMAJ(
                "tarifs", [("IDcategorie_tarif", None),], "IDtarif", IDtarif)

            # Remplit le nouveau champ IDcategorie_tarif de la table Prestations
            DB.ReqMAJ("prestations", [
                      ("IDcategorie_tarif", IDcategorie_tarif),], "IDtarif", IDtarif)
    ...
    for IDnom_tarif, IDactivite, IDcategorie_tarif, nom in listeNomsTarifs:
        ...
        DB.ReqMAJ("noms_tarifs", [
                  ("IDcategorie_tarif", None),], "IDnom_tarif", IDnom_tarif)
        ...
        for nom, listeIDnom_tarif in dictNoms.items():
            ...
            if len(listeIDnom_tarif) > 1:
                for IDnom_tarif in listeIDnom_tarif[1:]:
                    ...
                    DB.ReqMAJ(
                        "tarifs", [("IDnom_tarif", newIDnom_tarif),], "IDnom_tarif", IDnom_tarif)
...
def E4072():
    ...
    dlg = wx.MultiChoiceDialog(None, message, _(
        u"Suppression des prestations sans consommations associées"), listeLabels)
    dlg.SetSize((450, 500))
    dlg.CenterOnScreen()
    if dlg.ShowModal() == wx.ID_OK:
...
def A5000():
    ...
    dlg = wx.TextEntryDialog(
        None, _(u"Saisissez la requête :"), _(u"Requête"), "")
    if dlg.ShowModal() == wx.ID_OK:
        req = dlg.GetValue()
        dlg.Destroy()
    else:
...
def A5200():
    ...
    dlgAttente = wx.BusyInfo(
        _(u"Veuillez patienter durant la procédure... Celle-ci peut nécessiter quelques minutes..."), None)
    if 'phoenix' not in wx.PlatformInfo:
        wx.Yield()
    ...
    for IDprestation, montant in listePrestations:
        EcritStatusbar(
            _(u"Correction des arrondis... Merci de patienter...             -> %d %% effectués") % (index * 100 / total))
        DB.ReqMAJ("prestations", [("montant", FloatToDecimal(
            montant, plusProche=True)),], "IDprestation", IDprestation)
        index += 1

    for IDventilation, montant in listeVentilations:
        EcritStatusbar(
            _(u"Correction des arrondis... Merci de patienter...             - > %d %% effectués") % (index * 100 / total))
        DB.ReqMAJ("ventilation", [("montant", FloatToDecimal(
            montant, plusProche=True)),], "IDventilation", IDventilation)
        index += 1
...
def A5300():
    ...
    for IDgroupe_activite, IDtype_groupe_activite, IDactivite in listeRegroupements:
        ...
        if key in listeTraites:
            DB.ReqDEL("groupes_activites",
                      "IDgroupe_activite", IDgroupe_activite)
        else:
...
def A5500():
    ...
    for IDfacture, IDcompte_payeur, date_debut, date_fin in listeFactures:
        ...
        dictFactures[IDcompte_payeur].append({"IDfacture": IDfacture, "date_debut": UTILS_Dates.DateEngEnDateDD(
            date_debut), "date_fin": UTILS_Dates.DateEngEnDateDD(date_fin)})
    ...
    dlg = wx.MessageDialog(None, _(u"Souhaitez-vous vraiment lancer cette procédure pour %d prestations ?\n\n(Vous pourrez suivre la progression dans la barre d'état)") %
                           nbrePrestations, _(u"Confirmation"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_QUESTION)
        ...
        if IDcompte_payeur in dictFactures:
            for dictFacture in dictFactures[IDcompte_payeur]:
                if date >= dictFacture["date_debut"] and date <= dictFacture["date_fin"]:
                    ...
                    DB.ReqMAJ("prestations", [
                              ("IDfacture", IDfacture),], "IDprestation", IDprestation)
# print "Traitement en cours...   %d/%d ....  Prestation ID%d -> Facture ID%d" % (index, nbrePrestations, IDprestation, IDfacture)
                    EcritStatusbar(_(u"Traitement en cours...   %d/%d ....  Prestation ID%d -> Facture ID%d") %
                                   (index, nbrePrestations, IDprestation, IDfacture))
                    nbreSucces += 1
                    time.sleep(0.05)
    ...
    # Fin du traitement
    dlg = wx.MessageDialog(None, _(u"Procédure terminée !\n\n%d prestations sur %s ont été rattachées avec succès !") % (
        nbreSucces, nbrePrestations), _(u"Fin"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
def A7650():
    ...
    for IDfamille, IDindividu, IDcivilite, nom, prenom in listeDonnees:
        ...
        dictIndividus[IDfamille].append(
            {"IDcivilite": IDcivilite, "IDindividu": IDindividu, "nom": nom, "prenom": prenom})
    ...
    print("Nbre de titulaires HELIOS a enregistrer : %d" %
          len(listeModifications))

    # Enregistrement des modifications
    DB = GestionDB.DB()
    DB.Executermany("UPDATE familles SET titulaire_helios=? WHERE IDfamille=?",
...
def A8120():
    ...
    DB = GestionDB.DB()
    DB.ExecuterReq(
        "UPDATE modes_reglements SET type_comptable='caisse' WHERE IDmode=1")
    DB.ExecuterReq(
        "UPDATE modes_reglements SET type_comptable='banque' WHERE IDmode<>1")
    DB.Commit()
    DB.Close()
...
def A8574():
    ...
    if valide == False:
        dlg = wx.MessageDialog(None, _(u"Impossible d'effectuer la procédure !\n\nLe numéro de version que vous avez saisi semble erroné. Vérifiez qu'il est formaté de la façon suivante : 'x.x.x.x'"), _(
            u"Erreur"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return False
...
def A8623():
    ...
    for IDventilation, IDexercice, date_budget, date_debut_exercice in listeDonnees:
        DB.ReqMAJ("compta_ventilation", [
                  ("date_budget", date_debut_exercice),], "IDventilation", IDventilation)
    ...
    for IDbudget, IDexercice, date_debut, date_fin, date_debut_exercice, date_fin_exercice in listeDonnees:
        DB.ReqMAJ("compta_budgets", [("date_debut", date_debut_exercice),
                  ("date_fin", date_fin_exercice)], "IDbudget", IDbudget)
...
def A8733():
    ...
    for IDinscription, IDindividu, IDactivite, IDcompte_payeur in listeDonnees:
        dictInscriptions[(IDindividu, IDactivite,
                          IDcompte_payeur)] = IDinscription
    ...
    for IDconso, IDindividu, IDactivite, IDcompte_payeur in listeDonnees:
        if (IDindividu, IDactivite, IDcompte_payeur) in dictInscriptions:
            IDinscription = dictInscriptions[(
                IDindividu, IDactivite, IDcompte_payeur)]
    ...
    if len(listeModifications) > 0:
        DB.Executermany(_(u"UPDATE consommations SET IDinscription=? WHERE IDconso=?"),
                        listeModifications, commit=False)
        DB.Commit()
...
def A8941():
    ...
    DB = GestionDB.DB()
    DB.ExecuterReq(
        """UPDATE tarifs SET etats='reservation;present;absenti';""")
    DB.Commit()
    DB.Close()
...
def A8956():
    ...
    for IDprestation, IDcompte_payeur, date, montant, IDfacture in listeDonnees:
        ...
        date = UTILS_Dates.DateEngEnDateDD(date)
        dictTemp = {"IDprestation": IDprestation, "IDcompte_payeur": IDcompte_payeur,
                    "date": date, "montant": montant, "IDfacture": IDfacture}
        ...
        if (IDfacture in dictPrestationsFactures) == False:
            dictPrestationsFactures[IDfacture] = {
                "total": FloatToDecimal(0.0), "prestations": []}
            ...
            if IDcompte_payeur in dictPrestations:
                ...
                if total_facture == totalPrestationsTrouvees:
                    for dictPrestation in listePrestationsTrouvees:
                        DB.ReqMAJ("prestations", [
                                  ("IDfacture", IDfacture),], "IDprestation", dictPrestation["IDprestation"])
    ...
    # Message de fin
    dlg = wx.MessageDialog(None, _(u"Résultats :\n\nNombre de factures réparées = %d\nNombre de prestations réparées = %d" % (
        len(listeFacturesReparees), len(listePrestationsReparees))), _(u"Fin de la procédure"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
def A8971():
    ...
    DB.ExecuterReq("UPDATE quotients SET IDtype_quotient=%d;" %
                   IDtype_quotient)
    DB.Commit()
    DB.Close()
...
def A9023():
    ...
    print("Procedure A9023 : Nbre ouvertures a corriger =",
          len(liste_modifications))

    # Enoi des modifications à la DB
    DB.Executermany("UPDATE ouvertures SET IDgroupe=? WHERE IDouverture=?",
                    liste_modifications, commit=True)
    DB.Close()
...
def A9054():
    ...
    DB = GestionDB.DB(nomFichier=Chemins.GetStaticPath(
        "Databases/Defaut.dat"), suffixe=None)
...
def A9073():
    ...
    DB.Executermany("UPDATE utilisateurs SET mdp=? WHERE IDutilisateur=?",
                    liste_modifications, commit=True)
    DB.Close()
...
def A9074():
    ...
    # Enregistrement des mots de passe cryptés
    DB.Executermany("UPDATE utilisateurs SET mdpcrypt=? WHERE IDutilisateur=?",
                    liste_modifications, commit=True)
    DB.Close()
...
def A9075():
    ...
    DB = GestionDB.DB()
    DB.Executermany("UPDATE utilisateurs SET mdp=NULL WHERE IDutilisateur<>?", [
                    (9999999,),], commit=True)
    DB.Close()
...
def A9081():
    ...
    DB = GestionDB.DB()
    IDprofil = DB.ReqInsert("profils", [(
        "label", u"Liste par défaut"), ("categorie", "impression_infos_medicales"), ("defaut", 1)])
    DB.ReqInsert("profils_parametres", [("IDprofil", IDprofil), ("nom", "colonnes"), ("type_donnee", "autre"), (
        "parametre", "[(u'Informations alimentaires', '2'), (u'Autres informations', '0')]")])
    DB.Close()
...
def A9102():
    ...
    DB.ExecuterReq(
        "UPDATE familles SET num_allocataire='', internet_actif=0, internet_identifiant='', internet_mdp='';")
    ...
    for IDindividu, nom, prenom in listeDonnees:
        ...
        liste_modifications.append((nom, prenom, num_secu, date_naiss, rue_resid, cp_resid, ville_resid,
                                   employeur, travail_tel, travail_mail, tel_domicile, tel_mobile, mail, IDindividu))
...
def A9122():
    ...
    for IDprestation, IDindividu, date, label, IDtarif, montant, IDfacture in listePrestations:
        dict_prestations[IDprestation] = {"IDprestation": IDprestation, "label": label, "IDindividu": IDindividu,
                                          "date": date, "IDtarif": IDtarif, "montant": FloatToDecimal(montant), "IDfacture": IDfacture}
    ...
    for IDconso, IDindividu, IDprestation, date, IDunite, nom_unite in listeConsommations:
        dict_consos[IDconso] = {"IDindividu": IDindividu, "IDprestation": IDprestation,
                                "date": date, "IDunite": IDunite, "nom_unite": nom_unite}
        dict_IDprestation_consos[IDprestation] = None
        dict_consos_regroupements = DictionnaireImbrique(
            dictionnaire=dict_consos_regroupements, cles=[IDindividu, date, IDunite], valeur=IDconso)
                    ...
                    if len(combi_unites) > 0:
                        ...
                        if combi_found == True:
                            for IDunite in combi_unites:
                                ...
                                dict_rapprochements = DictionnaireImbrique(dictionnaire=dict_rapprochements, cles=[
                                                                           IDindividu, date, IDprestation], valeur=[])
                                dict_rapprochements[IDindividu][date][IDprestation].append(
                                    IDconso)
                                liste_IDprestation_corrigees.append(
                                    IDprestation)
            ...
            for IDprestation, liste_consos in dict_prestations_temp.items():
                for IDconso in liste_consos:
                    DB.ReqMAJ("consommations", [
                              ("IDprestation", IDprestation), ], "IDconso", IDconso)

    DB.Close()

    # Message de fin
    dlg = wx.MessageDialog(None, _(u"Réparation terminée : %d prestations corrigées.") % len(
        liste_IDprestation_corrigees), _(u"Fin de la procédure"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
def A9135():
    ...
    for IDfacture, IDfamille, date_debut, date_fin, total, nbre_prestations, total_prestations in listeFactures:
        ...
        if total != total_prestations:
            texte = u"Famille %s : Facture du %s au %s (Montant initial = %s / Montant actuel = %s)" % (
                famille, date_debut, date_fin, total, total_prestations)
            liste_anomalies.append((IDfacture, IDfamille, famille, date_debut,
                                   date_fin, total, total_prestations, nbre_prestations, texte))
...
def A9007():
    ...
    DB.Commit()
    DB.ExecuterReq(
        """UPDATE individus SET IDnationalite=73 WHERE IDnationalite IS NULL;""")
    DB.Commit()
    DB.ExecuterReq(
        """UPDATE individus SET IDpays_naiss=73 WHERE IDpays_naiss IS NULL;""")
    DB.Commit()
    DB.Close()
...
def A9012():
    ...
    for IDinscription, IDindividu, IDfamille, IDactivite, IDgroupe, IDcategorie_tarif, IDcompte_payeur, date_inscription, parti, date_desinscription, statut in listeInscriptions:
        ...
        data = [("IDindividu", IDindividu), ("IDfamille", IDfamille), ("IDactivite", IDactivite), ("IDgroupe", IDgroupe),
                ("IDcategorie_tarif", IDcategorie_tarif), ("IDcompte_payeur",
                                                           IDcompte_payeur), ("date_inscription", date_inscription),
                ("parti", parti), ("date_desinscription",
                                   date_desinscription), ("statut", statut),
                ]
        DB.ReqInsert("inscriptions", data)
        EcritStatusbar("Veuillez patienter durant le traitement. %d / %d inscriptions..." %
                       (index, len(listeInscriptions)))
        index += 1
...
def A9038():
    ...
    EcritStatusbar(u"%s actions de l'historique à modifier" %
                   len(liste_modifications))
    # Modification du IDdonnee dans l'historique
    for IDaction, IDlocation in liste_modifications:
        DB.ReqMAJ("historique", [
                  ("IDdonnee", IDlocation),], "IDaction", IDaction)
    DB.Close()
    EcritStatusbar("")
...
def A9050():
    ...
    for IDfamille, internet_mdp in listeFamilles:
        if not internet_mdp.startswith("custom") and not internet_mdp.startswith("#@#"):
            internet_mdp = UTILS_Internet.CrypteMDP(
                internet_mdp, IDfichier=IDfichier)
            liste_modifications.append((internet_mdp, IDfamille))
    DB.Executermany("UPDATE familles SET internet_mdp=? WHERE IDfamille=?",
                    liste_modifications, commit=True)
    ...
    DB.Executermany("UPDATE utilisateurs SET internet_mdp=? WHERE IDutilisateur=?",
                    liste_modifications, commit=True)
    ...
    for IDpaiement, liste_actions in dict_resultats.items():
        if liste_actions:
            ...
            if len(liste_actions) == 0:
                ...
                condition = "(%d)" % liste_actions[0]
            else:
                condition = str(tuple(liste_actions))
            DB.ExecuterReq(
                "UPDATE portail_actions SET etat='validation' WHERE IDaction IN %s" % condition)
            DB.Commit()
    DB.Close()
...
def A9055():
    ...
    """
    dlg = wx.TextEntryDialog(None, _(
        u"Saisissez le nom de la table à créer :"), _(u"Création d'une table"), "")
    reponse = dlg.ShowModal()
    ...
    if DB.IsTableExists(nom_table) == False:
        ...
        dlg = wx.MessageDialog(None, _(u"Cette table existe déjà !"), _(
            u"Erreur"), wx.OK | wx.ICON_ERROR)
        dlg.ShowModal()
        dlg.Destroy()
        return
...
def A9062():
    ...
    DB.ExecuterReq("ALTER TABLE %s MODIFY %s %s" %
                   (nom_table, nom_champ, type_champ))
    DB.Commit()
    DB.Close()
...
def A9064():
    ...
    print("Nbre de tiers solidaires a enregistrer : %d" %
          len(listeModifications))

    # Enregistrement des modifications
    DB = GestionDB.DB()
    DB.Executermany("UPDATE familles SET tiers_solidaire=? WHERE IDfamille=?",
                    listeModifications, commit=False)
    DB.Commit()
    DB.Close()
...
def A9068():
    ...
    dlg = wx.SingleChoiceDialog(None, u"Sélectionnez une activité :", u"Désinscription par lot", [
                                nom for idactivite, nom in listeActivites], wx.CHOICEDLG_STYLE)
    dlg.SetSize((400, 400))
    dlg.CenterOnScreen()
    if dlg.ShowModal() == wx.ID_OK:
    ...
    DB.ExecuterReq(req)
    listeInscriptions = DB.ResultatReq()
    dlg = wx.MultiChoiceDialog(None, u"Cochez les individus à désinscrire", u"Désinscription par lot", [
                               "%s %s" % (nom, prenom) for idinscription, nom, prenom in listeInscriptions], wx.CHOICEDLG_STYLE)
    dlg.SetSize((400, 400))
    dlg.CenterOnScreen()
    if dlg.ShowModal() == wx.ID_OK:
    ...
    dlg = wx.TextEntryDialog(
        None, u"Saisissez la date de désinscription au format JJ/MM/AAAA :", u"Désinscription par lot", "")
    reponse = dlg.ShowModal()
    date = dlg.GetValue()
    if reponse != wx.ID_OK:
        DB.Close()
    ...
    DB.ExecuterReq("UPDATE inscriptions SET date_desinscription='%s' WHERE %s;" % (
        date_desinscription, condition))
    DB.Commit()
    DB.Close()
...
def A9070():
    ...
    dlg = wx.SingleChoiceDialog(None, u"Sélectionnez une unité multihoraires à convertir en standard :", u"Conversion", [
                                "%s (%s)" % (nom_unite, nom_activite) for IDunite, nom_unite, nom_activite in listeUnites], wx.CHOICEDLG_STYLE)
    dlg.SetSize((400, 400))
    dlg.CenterOnScreen()
    if dlg.ShowModal() == wx.ID_OK:
        index = dlg.GetSelection()
    ...
    DB.ExecuterReq(
        "UPDATE unites SET type='Unitaire' WHERE IDunite=%d;" % IDunite)
    DB.Commit()
    DB.Close()
...
def A9072():
    ...
    DB.ExecuterReq(req)
    listeActivites = DB.ResultatReq()
    dlg = wx.SingleChoiceDialog(None, u"Sélectionnez une activité :", u"Choix de l'activité", [
                                nom_activite for IDactivite, nom_activite in listeActivites], wx.CHOICEDLG_STYLE)
    dlg.SetSize((400, 400))
    dlg.CenterOnScreen()
    if dlg.ShowModal() == wx.ID_OK:
        index = dlg.GetSelection()
    ...
    req = """SELECT IDprestation, IDindividu FROM prestations WHERE montant=1 AND IDactivite=%d AND date>='%s' AND date<='%s';""" % (
        IDactivite, date_debut, date_fin)
    DB.ExecuterReq(req)
    listePrestations = DB.ResultatReq()
    DB.Close()
    liste_individus = []
    ...
    txt_resultats = _(u"Résultats : %d prestations / %d individus.") % (
        len(listePrestations), len(liste_individus))
    dlg = wx.MessageDialog(None, txt_resultats, _(
        u"Résultats"), wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
    return
...
def A9074():
    ...
    liste_activites = (49, 47, 40, 20, 32, 70, 33, 46, 34,
                       85, 84, 75, 74, 71, 68, 67, 29, 56, 37, 63, 59, 57)

    DB = GestionDB.DB()
    DB.ExecuterReq(
        """DELETE FROM consommations WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.ExecuterReq(
        """DELETE FROM inscriptions WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.ExecuterReq("""DELETE FROM unites WHERE IDactivite IN %s;""" %
                   str(liste_activites))
    DB.ExecuterReq(
        """DELETE FROM unites_remplissage WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.ExecuterReq(
        """DELETE FROM activites WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.ExecuterReq(
        """DELETE FROM categories_tarifs WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.ExecuterReq(
        """DELETE FROM responsables_activite WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.ExecuterReq("""DELETE FROM groupes WHERE IDactivite IN %s;""" %
                   str(liste_activites))
    DB.ExecuterReq(
        """DELETE FROM unites_remplissage WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.ExecuterReq(
        """DELETE FROM ouvertures WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.ExecuterReq(
        """DELETE FROM remplissage WHERE IDactivite IN %s;""" % str(liste_activites))
    DB.Commit()
    DB.Close()
...
def A9277():
    ...
    DB = GestionDB.DB()
    DB.ExecuterReq(
        "ALTER TABLE activites ADD COLUMN code_service VARCHAR(200)")
    DB.Commit()
    DB.Close()
...
def A9279():
    ...
    for IDfacture, montant_facture in listePrestations:
        ...
        if ventilation > montant_facture:
            ...
            print("Anomalie sur facture ID%d : total facture = %.2f | Ventilation = %.2f" % (
                IDfacture, float(montant_facture), float(ventilation)))
