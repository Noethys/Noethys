...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):            
        ...
        liste_Colonnes = [
            ...
            ColumnDefn(_(u"Nbre arrêts"), "center", 100, "nbreArrets", typeDonnee="entier"),
            ]
    ...
    def Ajouter(self, event):
        ...
        dlg = wx.TextEntryDialog(self, _(u"Saisissez le nom de la nouvelle %s :") % self.categorieSingulier, _(u"Saisie d'une nouvelle %s") % self.categorieSingulier, u"")
    ...
    def Modifier(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _(u"Vous n'avez sélectionné aucune %s à modifier !") % self.categorieSingulier, _(u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        IDligne = self.Selection()[0].IDligne
        nom = self.Selection()[0].nom
        dlg = wx.TextEntryDialog(self, _(u"Modifiez le nom de la %s :") % self.categorieSingulier, _(u"Modification d'une %s") % self.categorieSingulier, nom)
        if dlg.ShowModal() == wx.ID_OK:
            nom = dlg.GetValue()
            if nom == "":
                dlg = wx.MessageDialog(self, _(u"Le nom que vous avez saisi n'est pas valide."), _(u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
    ...
    def Supprimer(self, event):
        ...
        if len(self.Selection()) == 0 :
            dlg = wx.MessageDialog(self, _(u"Vous n'avez sélectionné aucune %s à supprimer !") % self.categorieSingulier, _(u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        if self.Selection()[0].nbreArrets > 0 :
            dlg = wx.MessageDialog(self, _(u"Il est impossible de supprimer une ligne qui a déjà un ou plusieurs arrêts !"), _(u"Suppression impossible"), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        dlg = wx.MessageDialog(self, _(u"Souhaitez-vous vraiment supprimer cette %s ?") % self.categorieSingulier, _(u"Suppression"), wx.YES_NO|wx.NO_DEFAULT|wx.CANCEL|wx.ICON_INFORMATION)
...
class BarreRecherche(wx.SearchCtrl):
    def __init__(self, parent):
        ...
        self.SetDescriptiveText(_(u"Rechercher une ligne..."))
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.myOlv = ListView(panel, id=-1, categorie="bus", categorieSingulier=_(u"ligne de bus"), categoriePluriel=_(u"lignes de bus"), name="OL_test", style=wx.LC_REPORT|wx.SUNKEN_BORDER|wx.LC_SINGLE_SEL|wx.LC_HRULES|wx.LC_VRULES)
        self.myOlv.MAJ() 
