...
def DateComplete(dateDD):
    ...
    listeJours = (_(u"Lundi"), _(u"Mardi"), _(u"Mercredi"), _(
        u"Jeudi"), _(u"Vendredi"), _(u"Samedi"), _(u"Dimanche"))
    listeMois = (_(u"janvier"), _(u"février"), _(u"mars"), _(u"avril"), _(u"mai"), _(u"juin"), _(
        u"juillet"), _(u"août"), _(u"septembre"), _(u"octobre"), _(u"novembre"), _(u"décembre"))
    dateComplete = listeJours[dateDD.weekday(
    )] + " " + str(dateDD.day) + " " + listeMois[dateDD.month-1] + " " + str(dateDD.year)
    return dateComplete
...
class ListView(FastObjectListView):
    ...
    def InitObjectListView(self):
        ...
        # Image list
        self.imgVert = self.AddNamedImages("ok", wx.Bitmap(Chemins.GetStaticPath(
            "Images/16x16/Ventilation_vert.png"), wx.BITMAP_TYPE_PNG))
        self.imgRouge = self.AddNamedImages("erreur", wx.Bitmap(Chemins.GetStaticPath(
            "Images/16x16/Ventilation_rouge.png"), wx.BITMAP_TYPE_PNG))
        self.imgOrange = self.AddNamedImages("addition", wx.Bitmap(
            Chemins.GetStaticPath("Images/16x16/Ventilation_orange.png"), wx.BITMAP_TYPE_PNG))

        self.imgAttente = self.AddNamedImages("attente", wx.Bitmap(
            Chemins.GetStaticPath("Images/16x16/Attente.png"), wx.BITMAP_TYPE_PNG))
        self.imgOk = self.AddNamedImages("ok", wx.Bitmap(
            Chemins.GetStaticPath("Images/16x16/Ok.png"), wx.BITMAP_TYPE_PNG))
        self.imgNon = self.AddNamedImages("erreur", wx.Bitmap(
        ...
        def GetImageVentilation(track):
            ...
            resteAVentiler = FloatToDecimal(
                track.montant) - FloatToDecimal(track.montant_ventilation)
        ...
        liste_Colonnes = [
            ColumnDefn(_(u"ID"), "left", 0, "IDreglement",
                       typeDonnee="entier"),
            ColumnDefn(_(u"Date"), 'left', 160, "date",
                       typeDonnee="date", stringConverter=FormateDateLong),
            ColumnDefn(_(u"Mode"), 'left', 110,
                       "nom_mode", typeDonnee="texte"),
            ColumnDefn(_(u"Emetteur"), 'left', 120,
                       "nom_emetteur", typeDonnee="texte"),
            ColumnDefn(_(u"Numéro"), 'left', 60,
                       "numero_piece", typeDonnee="texte"),
            ColumnDefn(_(u"Payeur"), 'left', 130,
                       "nom_payeur", typeDonnee="texte"),
            ColumnDefn(_(u"Montant"), 'right', 60, "montant",
                       typeDonnee="montant", stringConverter=FormateMontant),
            ColumnDefn(_(u"Ventilé"), 'right', 80, "montant_ventilation", typeDonnee="montant",
                       stringConverter=FormateMontant, imageGetter=GetImageVentilation),
            ColumnDefn(_(u"Dépôt"), 'left', 90, "date_depot", typeDonnee="date",
                       stringConverter=FormateDateCourt, imageGetter=GetImageDepot),
            ColumnDefn(_(u"Observations"), 'left', 130,
                       "observations", typeDonnee="texte"),
        ]

        if self.mode == "liste":
            liste_Colonnes.insert(5, ColumnDefn(
                _(u"Famille"), 'left', 130, "nomTitulaires", typeDonnee="texte"))
...
# self.SetSortColumn(self.columns[1])
    ...
    def OnContextMenu(self, event):
        ...
        if self.mode != "liste":
            ...
            item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath(
        ...
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath(
            "Images/16x16/Remboursement.png"), wx.BITMAP_TYPE_PNG))
        menuPop.AppendItem(item)
        ...
        item = wx.MenuItem(sousMenuVentilation, 201, _(
            u"Uniquement le règlement sélectionné"))
        item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath(
            "Images/16x16/Magique.png"), wx.BITMAP_TYPE_PNG))
        ...
        menuPop.AppendMenu(wx.Window.NewControlId(), _(
            u"Ventilation automatique"), sousMenuVentilation)
    ...
    def EditerRecu(self, event):
        if len(self.Selection()) == 0:
            dlg = wx.MessageDialog(self, _(u"Vous n'avez sélectionné aucun règlement dans la liste !"), _(
    ...
    def GetDetailReglements(self):
        ...
        for track in self.donnees:
            if track.inclus == True:
                ...
                if (track.IDmode in dictDetails) == False:
                    dictDetails[track.IDmode] = {
                        "label": track.nom_mode, "nbre": 0, "montant": 0.0}
        ...
        for IDmode, dictDetail in dictDetails.items():
            texteDetail = u"%d %s (%.2f €), " % (
                dictDetail["nbre"], dictDetail["label"], dictDetail["montant"])
            texte += texteDetail
    ...
    def Ajouter(self, event):
        if self.IDcompte_payeur != None and UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("familles_reglements", "creer") == False:
            return

        from Dlg import DLG_Saisie_reglement
        dlg = DLG_Saisie_reglement.Dialog(
            self, IDcompte_payeur=self.IDcompte_payeur, IDreglement=None)
    ...
    def Modifier(self, event):
        if self.IDcompte_payeur != None and UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("familles_reglements", "modifier") == False:
            return

        if len(self.Selection()) == 0:
            dlg = wx.MessageDialog(self, _(u"Vous n'avez sélectionné aucun règlement à modifier dans la liste !"), _(
                u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        from Dlg import DLG_Saisie_reglement
        dlg = DLG_Saisie_reglement.Dialog(
            self, IDcompte_payeur=self.IDcompte_payeur, IDreglement=IDreglement)
        if dlg.ShowModal() == wx.ID_OK:
            self.MAJ(IDreglement)
        dlg.Destroy()

    def Supprimer(self, event):
        if self.IDcompte_payeur != None and UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("familles_reglements", "supprimer") == False:
            return

        if len(self.Selection()) == 0:
            dlg = wx.MessageDialog(self, _(u"Vous n'avez sélectionné aucun règlement à supprimer dans la liste"), _(
                u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        if IDdepot != None:
            dlg = wx.MessageDialog(self, _(u"Ce règlement est déjà attribué à un dépôt. Vous ne pouvez donc pas le supprimer !"), _(
                u"Règlement déposé"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        ...
        # Avertissement si appartient à un prélèvement
        if self.Selection()[0].IDprelevement != None:
            dlg = wx.MessageDialog(self, _(u"Ce règlement est rattaché à un prélèvement automatique.\n\nSouhaitez-vous vraiment le supprimer ?"), _(
                u"Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
            reponse = dlg.ShowModal()
            dlg.Destroy()
            if reponse != wx.ID_YES:
        ...
        if len(listeFrais) > 0:
            ...
            dlg = wx.MessageDialog(self, _(u"Une prestation d'un montant de %.2f %s pour frais de gestion est associée à ce règlement. Cette prestation sera automatiquement supprimée en même temps que le règlement.\n\nConfirmez-vous tout de même la suppression de ce règlement ?") %
                                   (montantFrais, SYMBOLE), _(u"Avertissement"), wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_EXCLAMATION)
    ...
    def VentilationAuto(self, event):
        ...
        if ID == 201:
            ...
            if len(self.Selection()) == 0:
                dlg = wx.MessageDialog(self, _(u"Vous n'avez sélectionné aucun règlement !"), _(
        ...
        if ID == 202:
            if len(self.donnees) == 0:
                dlg = wx.MessageDialog(self, _(u"La liste des règlements est vide !"), _(
                    u"Erreur"), wx.OK | wx.ICON_EXCLAMATION)
                dlg.ShowModal()
                dlg.Destroy()
                return
    ...
    def Rembourser(self, event=None):
        if UTILS_Utilisateurs.VerificationDroitsUtilisateurActuel("familles_remboursements", "creer") == False:
            return
        ...
        if solde >= 0.0:
            dlg = wx.MessageDialog(self, _(u"Il est impossible de créer un remboursement car il n'y a pas d'avoir !\n\nLe solde du compte est de %.2f %s.") %
                                   (-solde, SYMBOLE), _(u"Erreur"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False

        dlg = DLG_Saisie_remboursement.Dialog(
            self, IDcompte_payeur=self.IDcompte_payeur, solde=-solde)
        dlg.ShowModal()
        dlg.Destroy()
        self.MAJ()
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.myOlv = ListView(panel, id=-1, mode="liste", name="OL_test", style=wx.LC_REPORT |
                              wx.SUNKEN_BORDER | wx.LC_SINGLE_SEL | wx.LC_HRULES | wx.LC_VRULES)
        self.myOlv.MAJ()
