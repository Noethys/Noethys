...
##        self.dc.SetId(self.IDobjet)
        ...
        if self.selectionCadre == True :
            ...
            dc.SetPen(wx.Pen(couleurSelectionCadre, 1, wx.DOT))
            dc.DrawRoundedRectangleRect(wx.Rect(x-ecart, y-ecart, largeur+(ecart*2), hauteur+(ecart*2)), radius=5*self.zoom)
        
        # Dessin du cadre
        dc.SetBrush(wx.Brush(couleurFondBasCadre))
        dc.SetPen(wx.Pen(couleurBordCadre, 1))
        dc.DrawRoundedRectangleRect(wx.Rect(x, y, largeur, hauteur), radius=5*self.zoom)
        
        coordsSpline = [(x+1, y+(hauteur/3)), (x+(largeur/2.5), y+(hauteur/4.1)), (x+largeur-1, y+(hauteur/1.8))]
        dc.DrawSpline(coordsSpline)
        
        dc.SetBrush(wx.Brush(couleurFondHautCadre) )
        dc.FloodFill(x+5, y+5, couleurBordCadre, style=wx.FLOOD_BORDER )
        
        dc.SetPen(wx.Pen(couleurFondBasCadre, 1))
        dc.DrawSpline(coordsSpline)
        
        # Intégration de la photo
        if self.nomImage == None : self.nomImage = "Personne.png"
        nomFichier = Chemins.GetStaticPath("Images/128x128/%s" % self.nomImage)
        ...
        dc.DrawBitmap(bmp, x+paddingCadre, y+paddingCadre)
        self.bmp = bmp
        ...
        for texte, tailleFont, styleFont in self.listeTextes :
            ...
            if (posYtexte - y + hauteurTexte) < hauteurMaxiTexte :
                if largeurTexte > largeurMaxiTexte :
                    texte = self.AdapteLargeurTexte(dc, texte, largeurMaxiTexte)
                if texte == "#SPACER#" : texte = " "
                dc.DrawText(texte, posXtexte, posYtexte)
        ...
        if self.calendrierActif == True and self.zoom > 1 :
            ...
            dc.DrawBitmap(bmpConso, xBmpConso, yBmpConso)
        ...
        if self.titulaire == 1 :
            ...
            dc.DrawBitmap(bmpTitulaire, x+paddingCadre, y+paddingCadre+2)
            
        # Mémorisation dans le dictionnaire d'objets
##        self.dc.SetIdBounds(self.IDobjet, wx.Rect(x, y, largeur, hauteur)) 
        self.parent.dictObjets[self.IDobjet] = wx.Rect(x, y, largeur, hauteur)
...
##            dc.SetId(numCol)
            ...
            if paramFond.has_key(numCol) : 
                ...
                bmp = wx.Bitmap(Chemins.GetStaticPath("Images/Special/%s" % paramFond[numCol]["nomImage"]), wx.BITMAP_TYPE_PNG)
...
##                dc.SetId(wx.NewId())
    ...
    def RechercheObjet(self, x, y):
        listeResultats = []
        for IDobjet, r in self.dictObjets.iteritems() :
            if r.ContainsXY(x, y) :
                listeResultats.append(IDobjet)
        return listeResultats

    def RechercheCadre(self, x, y):
        """ Recherche le cadre présent sur x, y """
        listeObjets = self.RechercheObjet(x, y)
