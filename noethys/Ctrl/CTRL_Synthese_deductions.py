...
LISTE_MOIS = (_(u"janvier"), _(u"février"), _(u"mars"), _(u"avril"), _(u"mai"), _(u"juin"), _(
...
def DateComplete(dateDD):
    ...
    )] + " " + str(dateDD.day) + " " + LISTE_MOIS[dateDD.month-1] + " " + str(dateDD.year)
    return dateComplete
...
class CTRL(gridlib.Grid, glr.GridWithLabelRenderersMixin):
    ...
    def MAJ(self, date_debut=None, date_fin=None, listeActivites=[], affichage_regroupement="jour", affichage_caisse=None, labelParametres=u""):
        ...
        if self.date_debut != date_debut:
            self.infosIndividus = UTILS_Infos_individus.Informations(date_reference=date_debut, qf=True, inscriptions=False, messages=False,
                                                                     infosMedicales=False, cotisationsManquantes=False, piecesManquantes=False, questionnaires=True, scolarite=False)
        ...
        try:
            dlgAttente = wx.BusyInfo(
                _(u"Veuillez patienter durant la recherche des données..."), None)
    ...
    def InitGrid(self):
        ...
        for numLigne, regroupement in enumerate(listeRegroupement):
            ...
            for numColonne, (code_colonne, label_colonne) in enumerate(listeColonnes):
                ...
                if code_colonne == "num_allocataire":
                    valeur = valeurs["num_allocataire"]
                if code_colonne == "famille":
                    valeur = valeurs["famille"]
                if code_colonne == "montant_initial":
                    valeur = valeurs["montant_initial"]
                if code_colonne == "montant_deduction":
                    valeur = valeurs["montant_deduction"]
                if code_colonne == "montant_final":
                    valeur = valeurs["montant_final"]
                if code_colonne == "date_min":
                    valeur = UTILS_Dates.DateDDEnFr(
                        min(valeurs["liste_dates"]))
                if code_colonne == "date_max":
                    valeur = UTILS_Dates.DateDDEnFr(
                        max(valeurs["liste_dates"]))
                if code_colonne == "nbre_dates":
                    valeur = len(valeurs["liste_dates"])
                if code_colonne == "nbre_individus":
                    valeur = len(valeurs["liste_individus"])
    ...
    def Apercu(self):
        ...
        if self.GetNumberRows() == 0 or self.GetNumberCols() == 0:
            dlg = wx.MessageDialog(
                self, _(u"Il n'y a rien à imprimer !"), "Erreur", wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return None
        ...
        for col in range(0, nbreColonnes+1):
            if col == 0:
                largeursColonnes.append(largeurColonneLabel)
            else:
        ...
        listeStyles = [
            ...
            # Centre verticalement toutes les cases
            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
            # Centre les labels de colonne
        ...
        style_label_col = ParagraphStyle(
            name="label", fontName="Helvetica", alignment=1, fontSize=6, spaceAfter=0, leading=8, spaceBefore=0)
        valeursLigne = ["",]
        ...
        styleA = ParagraphStyle(
            name="A", fontName="Helvetica", alignment=1, fontSize=6, leading=7, spaceAfter=0)
        ...
        for numLigne in range(0, nbreLignes):
            ...
            for numCol in range(0, nbreColonnes):
                ...
                if couleurCase != (255, 255, 255, 255) and avecCouleurs == True:
                    ...
                    listeStyles.append(
                        ('BACKGROUND', (numCol+1, numLigne+1), (numCol+1, numLigne+1), (r, g, b)))
                # if numLigne == 0 :
        ...
        dataTableau.append((_(u"Synthèse des déductions"), _(
        ...
        styleA = ParagraphStyle(
            name="A", fontName="Helvetica", fontSize=6, spaceAfter=20)
        ...
        doc = SimpleDocTemplate(nomDoc, pagesize=(largeur, hauteur), leftMargin=tailleMarge,
                                rightMargin=tailleMarge, topMargin=tailleMarge, bottomMargin=tailleMarge)
        doc.build(story)
    ...
    def ExportTexte(self, event=None):
        ...
        UTILS_Export.ExportTexte(
            grid=self, titre=_(u"Synthèse des déductions"))
