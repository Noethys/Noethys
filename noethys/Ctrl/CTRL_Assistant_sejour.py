...
class Page_generalites(Assistant.Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(code="nom", titre=_(u"Quel est le nom du séjour ?"), commentaire=_(u"Exemple : 'Séjour neige - Février 2018'"), ctrl=Assistant.CTRL_Texte, obligatoire=True)
        self.Ajouter_question(code="date_debut", titre=_(u"Quelle est la date de début du séjour ?"), commentaire=None, ctrl=Assistant.CTRL_Date, obligatoire=True)
        self.Ajouter_question(code="date_fin", titre=_(u"Quelle est la date de fin du séjour ?"), commentaire=None, ctrl=Assistant.CTRL_Date, obligatoire=True)
        self.Ajouter_question(code="num_agrement", titre=_(u"Quel est le numéro d'agrément du séjour ?"), commentaire=_(u"Exemple : '012321CL0121'. S'il n'y a pas d'agrément, laissez vide."), ctrl=Assistant.CTRL_Texte)
        ...
        self.Ajouter_question(code="groupes_activites", titre=_(u"Cochez les groupes d'activités associés à ce séjour :"), commentaire=_(u"Les groupes d'activités permettent une sélection plus rapide dans certaines fenêtres de Noethys."), ctrl=Assistant.CTRL_Groupes_activite)

    def Suite(self):
        # Validation des dates du séjour
        if self.parent.dict_valeurs["date_debut"] > self.parent.dict_valeurs["date_fin"]:
            dlg = wx.MessageDialog(self, _(u"La date de fin du séjour doit être supérieure à la date du début !"), _(u"Erreur de saisie"), wx.OK | wx.ICON_EXCLAMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return False
...
class Page_groupes(Assistant.Page):
    def __init__(self, parent):
        ...
        self.Ajouter_question(code="has_groupes", titre=_(u"Le séjour est-il constitué de groupes distincts ?"), commentaire=_(u"Exemple : Les maternels et les primaires. Si vous n'êtes pas sûr, laissez non."), ctrl=Assistant.CTRL_Oui_non, defaut=False)
...
class Page_conclusion(Assistant.Page):
    def __init__(self, parent):
        Assistant.Page.__init__(self, parent)
        self.Ajouter_question(titre=_(u"Félicitations, vous avez terminé de paramétrer votre séjour !"))
