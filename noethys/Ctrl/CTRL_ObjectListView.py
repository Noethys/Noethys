...
class ObjectListView(OLV.ObjectListView):
    ...
    def AddColumnDefn(self, defn):
        # Enlï¿œve l'espace gauche sur tous les headers du listctrl sous Phoenix en ajoutant une image transparente
    ...
    def _HandleSize(self, evt):
        ...
        # Masque le texte "Aucun" si version phoenix (ï¿œ cause des colonnes bleues)
        if 'phoenix' in wx.PlatformInfo:
    ...
    def SetColumns2(self, colonnes=[], nomListe=None):
        """ Pour une liste avec possibilitï¿œs de configuration """
    ...
    def MenuConfigurerListe(self, event=None):
        ...
        for col in self.listeColonnesDefaut:
            if hasattr(col, "visible"):
                # Mï¿œmorise colonne disponible
                nom = col.title
                if nom == "":
                    nom = col.valueGetter
                colonnes_dispo.append({"nom" : nom, "code" : col.valueGetter, "col" : col})

        # Mï¿œmorise sï¿œlection actuelle
        colonnes_selection = []
        ...
        # Recherche les colonnes par dï¿œfaut
    ...
    def AjouteLigneTotal(self, listeNomsColonnes=[]):
        ...
        # Rï¿œcupï¿œration des totaux des colonnes souhaitï¿œes
        dictTotaux = {}
        nbreLignes = 0
...
##        self.modelObjects.append(track)
    ...
    def GetCotisations(self, mode="individu", choix="", criteres={}):
        """ Rï¿œcupï¿œration de la liste des individus inscrits et prï¿œsents """
        date = criteres["date"]
    ...
    def GenerationContextMenu(self, menu=None, intro="", total="", titre=None, orientation=wx.PORTRAIT, dictParametres=None):
        ...
        # Met ï¿œ jour le titre de la liste si besoin
        if titre != None :
            self.titre = titre
        ...
        if self.checkStateColumn != None:
            ...
            # Item Tout dï¿œcocher
            id = wx.Window.NewControlId()
            item = wx.MenuItem(menu, id, _(u"Tout dï¿œcocher"))
        ...
        id = wx.Window.NewControlId()
        item = wx.MenuItem(menu, id, _(u"Aperï¿œu avant impression"))
...
##        self.bouton_filtrage = wx.Button(self, -1, u"Filtrage avancï¿œ", size=(-1, 20))
##        self.bouton_filtrage = wx.BitmapButton(self, -1, wx.Bitmap("Images/BoutonsImages/Filtrer_liste_2.png", wx.BITMAP_TYPE_ANY))
        ...
        item = wx.MenuItem(menu, 10, u"Ajouter, modifier ou supprimer des filtres", u"Cliquez ici pour accï¿œder ï¿œ la gestion des filtres de listes")
        ...
        if afficherCocher == True :
            ...
            self.bouton_cocher.SetToolTip(wx.ToolTip(_(u"Cliquez ici pour cocher ou dï¿œcocher rapidement tous les ï¿œlï¿œments de cette liste")))
            
            menu = UTILS_Adaptations.Menu()
            item = wx.MenuItem(menu, 20, u"Tout cocher", u"Cliquez ici pour cocher tous les ï¿œlï¿œments de la liste")
            item.SetBitmap(wx.Bitmap(Chemins.GetStaticPath("Images/16x16/Cocher.png"), wx.BITMAP_TYPE_ANY))
            menu.AppendItem(item)
            item = wx.MenuItem(menu, 21, u"Tout dï¿œcocher", u"Cliquez ici pour dï¿œcocher tous les ï¿œlï¿œments de la liste")
    ...
    def MAJ_ctrl_filtrer(self):
        """ Met ï¿œ jour l'image du bouton Filtrage """
        nbreFiltres = len(self.listview.listeFiltresColonnes)
        
        # Modifie l'image selon le nbre de filtres activï¿œs
        if nbreFiltres == 0 :
            nomImage = "Filtre"
        elif nbreFiltres < 10 :
        ...
        # Modifie le tip en fonction des filtres activï¿œs
        if nbreFiltres == 0 :
            ...
            if nbreFiltres == 1 :
                ...
                texte = u"Cliquez ici pour filtrer cette liste\n> %d filtres activï¿œs" % nbreFiltres        
    ...
    def OnMenu(self, event):
        ID = event.GetId()
        # Accï¿œder ï¿œ la gestion des filtres
        if ID == 10 : 
